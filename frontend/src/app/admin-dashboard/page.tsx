...
export function AdminDashboard() {
  ...
  useEffect(() => {
    if (isAdmin) {
      fetch("https://liwandb.mavoid.com:5000/api/v1/departments/", {
        headers: {
  ...
  const handleAddDepartment = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://liwandb.mavoid.com:5000/api/v1/departments/",
        {
          method: "POST",
          headers: {
  ...
  const handleDeleteManager = async (extensionsnumber) => {
    ...
    try {
      ...
      .find((row) => row.startsWith("accessToken"))
      ?.split("=")[1];
      const response = await fetch('https://liwandb.mavoid.com:5000/api/v1/employees/UpdateManagerDep', {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`,
  ...
  const handleDeleteDepartment = async (departmentId) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://liwandb.mavoid.com:5000/api/v1/departments/${departmentId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${
